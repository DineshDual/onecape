...
export default function Testimonials() {
  return (
    <section id="testimonials" className="py-24 sm:py-32 px-6 bg-white" aria-labelledby="testimonials-heading">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="max-w-2xl mb-16">
          ...
          <h2 id="testimonials-heading" className="mt-4 text-4xl sm:text-5xl lg:text-6xl font-extrabold text-[#0D1B2A] leading-tight">
            Don&apos;t take our word.{" "}
        ...
        <div className="grid md:grid-cols-3 gap-6">
          {testimonials.map((t, idx) => (
            <article
              key={idx}
              className="testimonial-card glass-card p-8 flex flex-col justify-between transition-all duration-500 rounded-lg"
            >
              {/* Stars */}
              <div className="flex gap-1 mb-5" role="img" aria-label={`${t.rating} out of 5 stars`}>
                {[...Array(t.rating)].map((_, i) => (
                  <svg key={i} className="w-4 h-4 text-[#FF6600]" fill="currentColor" viewBox="0 0 20 20" aria-hidden="true">
              ...
              <blockquote className="text-gray-600 leading-relaxed text-[15px] flex-1">
                &ldquo;{t.quote}&rdquo;
              </blockquote>

              <div className="mt-8 flex items-center gap-4 pt-6 border-t border-[#E5E5E5]">
                <div className="w-11 h-11 rounded-full bg-[#FF6600]/10 flex items-center justify-center text-[#FF6600] text-sm font-bold border border-[#FF6600]/15" aria-hidden="true">
