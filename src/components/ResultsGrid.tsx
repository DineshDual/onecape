...
export default function ResultsGrid() {
  return (
    <section id="results" className="py-24 sm:py-32 px-6">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="overflow-x-auto -mx-6 px-6">
          {/* Header */}
          <div className="hidden lg:grid lg:grid-cols-[60px_1fr_140px_140px_140px] gap-4 px-6 py-4 border-b border-gray-800 text-xs text-gray-500 tracking-widest uppercase font-medium">
            <div>Rank</div>
            <div>Strategy</div>
            ...
            {results.map((result, idx) => (
              ...
              >
                ...
                <div className="flex items-center gap-3 mb-4 lg:mb-0">
                  <span className={`text-2xl font-black ${idx === 0 ? 'text-[#d4a853]' : idx === 1 ? 'text-[#e8c77b]' : 'text-gray-600'}`}>
                    {result.priority}
                  </span>
                  <div className="lg:hidden">
                    <span className="text-2xl font-black text-[#d4a853]">{result.revenue}</span>
                    <span className="text-sm text-[#d4a853]/70">{result.period}</span>
                  </div>
                </div>
                ...
                {/* Investment */}
                <div className="hidden lg:block">
                  <span className="text-gray-300 font-semibold">{result.investment}</span>
                </div>

                {/* Timeline */}
                <div className="hidden lg:block">
                  <span className="text-gray-400">{result.timeline}</span>
                </div>
              </div>
            ))}
          </div>
        ...
        <div className="mt-12 grid grid-cols-2 md:grid-cols-4 gap-4">
          {[
            ...
            <div key={stat.label} className="text-center p-4 border border-gray-800/50 rounded-lg">
              <div className="text-2xl sm:text-3xl font-black text-[#d4a853]">{stat.value}</div>
